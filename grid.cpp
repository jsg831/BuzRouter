...
void Grid::add_obstacles( const std::vector<Rectangle>& obstacles )
{
  for ( unsigned int o = 0; o < obstacles.size(); ++o ) {
    ...
    const unsigned int coor_int_low = safe_sub( obstacle.lower.coor[!dir], spacing );
    const unsigned int coor_int_upp = safe_add( obstacle.upper.coor[!dir], spacing );
  ...
  unsigned int spacing)
{
  ...
  for ( unsigned int p = 0; p < pin_shapes.size(); ++p ) {
    ...
    unsigned int t_node = find_upper_bound(t_pin_upp, sublayer.sltra_coor);
    unsigned int t_min_limit = find_upper_bound(t_pin_low - bw/2, sublayer.sltra_coor);
    bool success = 0;
    while ( t_node != -1 ) {
      const GridNode& grid_node = grid_nodes[t_node][i_node];
      const unsigned int& tra_coor = sublayer.sltra_coor[t_node];
      const unsigned int tra_upp = tra_coor + ( bw / 2 );
      const unsigned int tra_low = tra_coor - ( bw / 2 );
      ...
      if ( (tra_upp <= t_pin_upp && tra_upp >= t_pin_low) || (tra_low <= t_pin_upp && tra_low >= t_pin_low) || (tra_upp >= t_pin_upp && tra_low <= t_pin_low) ) {
        if ( grid_node.routable() && check_pinout_spacing(tra_upp, i_pin, l, sl,
          heading, direction, obstacles, spacing) ) {
...
bool Grid::check_pinout_spacing( const unsigned int t_coor, const unsigned int i_coor,
  unsigned char l, unsigned char sl, bool heading, bool direction, std::vector<Rectangle> obstacles,
  unsigned int spacing )
{
  for ( unsigned int n = 0; n < obstacles.size(); n ++ ) {
    Rectangle obs = obstacles[n];
    if ( !(obs.l == l && obs.sl == sl) ) continue;
    if ( overlap_node2rec_spacing( t_coor, i_coor, direction, obs, spacing) )
      return 0;
  }
  return 1;
}

bool Grid::overlap_node2rec_spacing( unsigned int t_coor, unsigned int i_coor,
  bool dir, Rectangle rec, unsigned int spacing )
{
  unsigned int rec_t_low = rec.lower.coor[dir] - spacing;
  unsigned int rec_t_upp = rec.upper.coor[dir] + spacing;
  unsigned int rec_i_low = rec.lower.coor[!dir];
  unsigned int rec_i_upp = rec.upper.coor[!dir];
  return  (t_coor >= rec_t_low) && (t_coor <= rec_t_upp) && (i_coor >= rec_i_low)
   && i_coor <= rec_i_upp;
}
